...
#define SOFTTIMER_ONE_SECOND_TICKS     (32768)
#define LCD_SERVICE_TIMER_HANDLE       (4)

uint8_t getLCDServiceTimerHandle()
{
  return LCD_SERVICE_TIMER_HANDLE;
}
...
void displayPrintf(enum display_row row, const char *format, ...)
{
   ...
   if (row >= DISPLAY_NUMBER_OF_ROWS) {
       LOG_ERROR("row parameter %d is greater than max row index %d\r\n", (int) row, (int) DISPLAY_NUMBER_OF_ROWS-1);
       return;
   }
   ...
   status = GLIB_drawStringOnLine(&display->glibContext,
                                   ...
                                   true);    // opaque
   if (status != GLIB_OK) {
       LOG_ERROR("Erase GLIB_drawStringOnLine() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
   }
   ...
   status = GLIB_drawStringOnLine(&display->glibContext,
                                  ...
                                  true);    // opaque
   if (status != GLIB_OK) {
       LOG_ERROR("Draw GLIB_drawStringOnLine() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
   }
   ...
   if (status != DMD_OK) {
       LOG_ERROR("DMD_updateDisplay() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
   }
...
void displayInit()
{
    ...
    status = DMD_init(0);
    if (status != DMD_OK) {
        LOG_ERROR("DMD_init() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
    }
    ...
    if (status != GLIB_OK) {
        LOG_ERROR("GLIB_contextInit() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
    }
    ...
    if (status != GLIB_OK) {
        LOG_ERROR("GLIB_clear() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
    }
    ...
    if (status != GLIB_OK) {
        LOG_ERROR("GLIB_setFont() returned non-zero error code=0x%04x\r\n", (unsigned int) status);
    }
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(SOFTTIMER_ONE_SECOND_TICKS, \
	                                               LCD_SERVICE_TIMER_HANDLE, false);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("Failed to create a soft timer, error = %d\r\n", timer_response);
     }
...
/**
 * Set up the LCD display when the device is booted.
 * @param device_name the name of the device that displays the identity information
 * @param addr the public BT address of the device
 * @param assignment_name
 * @param state the current state machine state to display
 */
void setup_display(const char *device_name, uint8_t *addr, \
                   const char *assignment_name, const char *state)
{
  // display the device name
  displayPrintf(DISPLAY_ROW_NAME, device_name);
  // display the public BT address
  displayPrintf(DISPLAY_ROW_BTADDR, "%s", addr);
  // display the assignment name
  displayPrintf(DISPLAY_ROW_ASSIGNMENT, assignment_name);
  // display advertising as the current connection state
  displayPrintf(DISPLAY_ROW_CONNECTION, state);
}
